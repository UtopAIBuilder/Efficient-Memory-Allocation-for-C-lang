...
if (input1)
{
	...
	while(input1 && carry!=0)
 	...
 	{
      temp->next=(struct node *)allocate();
      temp=temp->next;
      temp->val=carry;
      temp->next=NULL;
      return result;
...
struct polyNode* inputPoly()
{
	...
	do{
		...
		for (int i = 0; i <dummyNodes; ++i)
		{
   			...
   			p->next =(struct polyNode*)allocate();
   			p->next->prev=p;
   			p=p->next;
   			p->next=NULL;
   			p->deg=dummyNodesdeg--;
...
void polyDiv(struct polyNode* divisor,struct polyNode* dividend)
{
   ...
   for (int i = dividend->deg - divisor->deg; i >=0 ; --i)
   {
   	if (quotient==NULL)
   	{
   		quotient=(struct polyNode*)allocate();
   		quotient->prev=NULL;
   		quotient->next=NULL;
	...
	{
		tempQuotient->next =(struct polyNode*)allocate();
